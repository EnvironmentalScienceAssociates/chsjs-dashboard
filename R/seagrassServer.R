...
seagrassServer <- function(id){
  moduleServer(id, function(input, output, session) {
    ...
    output$tsPlot <- renderPlotly({
      ...
      y_lab = "Avg. Cover (%)"
      if (input$param == "BL_avg") y_lab = "Avg. Blade Length"
      if (input$param == "Depth_m") y_lab = "Avg. Depth (m)"
      
      p = ggplot(tsSumm(), aes(x = Year, y = Resp, col = Strata, 
                               group = Strata, text = Tooltip)) +
        geom_line() +
        geom_point() +
        labs(y = y_lab) +
